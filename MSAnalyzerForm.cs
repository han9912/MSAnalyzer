...
{
    public partial class MSAnalyzerForm : Form
    {
        string logRootDir = "";
        Thread thdCapture;
        DataTable mdt = null;
        ...
        readonly int valueCol = 10, firstValueRow = 2;

        public MSAnalyzerForm()
        {
            InitializeComponent();
            this.tb_TemplatePath.Text = templatePath;
        }
        ...
        private void Btn_SelectTemplate_Click(object sender, EventArgs e)
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK) tb_TemplatePath.Text = dialog.FileName;
        ...
        private void ThreadCaptureMethod()
        {
            ...
            if (this.InvokeRequired) this.BeginInvoke((MethodInvoker)(() => {
                btn_OpenReportPath_Click.Enabled = false;
            }));
            else btn_OpenReportPath_Click.Enabled = false;
            ...
            for (int k = 0; k < workBook.Worksheets.Count; k++) //遍历worksheet,次数相当于测试项目数
            {
                ...
                try
                {
                    mdt = GetDataTableFromFolder(logRootDir, k + firstValueRow, valueCol);
                }
                catch (Exception ex)
                {
                ...
                if (templatePath == "") worksheet = PrintToRawWorksheet(worksheet, mdt);
                else worksheet = PrintToWorksheet(worksheet, mdt);
            }
            ...
            else btn_OpenReportPath_Click.Enabled = true;
        }
        /// <summary>
        ...
        {
            XLWorkbook wb;
            if (templatePath != "") wb = new XLWorkbook(templatePath);
            else
            {
                wb = new XLWorkbook();
                wb.AddWorksheet();
        ...
        /// <summary>
        /// Print a DataTable to a raw Worksheet
        /// </summary>
        /// <param name="worksheet"></param>
        /// <param name="dt"></param>
        /// <returns></returns>
        private IXLWorksheet PrintToRawWorksheet(IXLWorksheet worksheet, DataTable dt)
        {
            int rowOffset = 2;
            int colOffset = 1;
            for (int i = 0; i < dt.Columns.Count; i++)
            {
                worksheet.Cell(1, i + colOffset).Value = dt.Columns[i].ColumnName;
                worksheet.Cell(1, i + colOffset).Style.Border.OutsideBorder = XLBorderStyleValues.Medium;
                for (int j = 0; j < dt.Rows.Count; j++)
                {
                    worksheet.Cell(j + rowOffset, i + colOffset).Value = dt.Rows[j][i].ToString();
                    worksheet.Cell(j + rowOffset, i + colOffset).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
                    worksheet.Cell(j + rowOffset, i + colOffset).Style.Alignment.Vertical = XLAlignmentVerticalValues.Center;
                }
            }
            return worksheet;
        }
        ...
        private string[] GetExpList()
        {
            int expCount = 0;
            string[] expList = new string[expCount];
            return expList;
        }

        private void Cb_Template_CheckedChanged(object sender, EventArgs e)
        {
            if (cb_Template.Checked == false)
            {
                templatePath = "";
